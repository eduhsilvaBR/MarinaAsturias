...
export default function Footer() {
  return (
    <footer className="border-t border-ink/10 bg-mist">
      <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 lg:grid-cols-4 lg:px-10">
        <div>
          <div className="relative block h-20 w-[63px]">
            <Image src="/logo/logo-lockup-dark.png" alt="Marina Astúrias" fill className="object-contain object-left" />
          </div>
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-ink/60">{siteInfo.boilerplate}</p>
          <div className="mt-6 flex gap-4 text-xs tracking-wide text-ink/60">
        ...
        <div>
          ...
          <ul className="mt-4 flex flex-col gap-2.5 text-sm text-ink/70">
            {navGroups.map((item) => (
              <li key={item.label}>
      ...
      <div className="border-t border-ink/10 px-6 py-6 text-center text-xs tracking-wide text-ink/40 lg:px-10">
